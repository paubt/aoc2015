    ...
    fn greedy_search(weighted_connection: HashMap<(String, String), i32>) -> i32 {
        ...
        while !remaining_persons.is_empty() {
            ...
            for &possible_neighbor in remaining_persons.iter() {
                ...
                if let Some(x) = weighted_connection
                    .get(&(last_added_person.to_string(), possible_neighbor.to_string()))
                {
                ...
                } else if let Some(x) = weighted_connection
                    .get(&(possible_neighbor.to_string(), last_added_person.to_string()))
                {
                    if *x > highest_happiness_viable_neighbor_value {
                        highest_happiness_viable_neighbor_value = *x;
                        highest_happiness_viable_neighbor = possible_neighbor;
        ...
        if let Some(x) = weighted_connection.get(&(
            final_order_of_persons.first().unwrap().to_string(),
            final_order_of_persons.last().unwrap().to_string(),
        )) {
            final_order_value += *x;
        } else if let Some(x) = weighted_connection.get(&(
            final_order_of_persons.last().unwrap().to_string(),
            final_order_of_persons.first().unwrap().to_string(),
        )) {
            final_order_value += *x;
